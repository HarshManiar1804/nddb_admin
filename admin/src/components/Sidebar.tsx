...
const Sidebar = () => {
  ...
  return (
    <div
      className={`relative h-screen bg-gray-50 border-r border-gray-200 left-0 top-0 pt-2 transition-all duration-300 ${isCollapsed ? 'w-20' : 'w-64'
        }`}
    >
      {/* Toggle button to collapse/expand the sidebar */}
      <button
        onClick={() => setIsCollapsed(!isCollapsed)}
        className="absolute -right-3 top-8 bg-gray-50 border border-gray-200 rounded-full p-1.5 hover:bg-gray-50 cursor-pointer"
      >
        {isCollapsed ? (
        ...
        {routes.map((route) => {
          ...
          return (
            <div key={route.path} className="relative group">
              ...
              <Link
                to={route.path}
                className={`flex items-center ${!isCollapsed ? 'space-x-3' : 'justify-center'} px-4 py-2.5 rounded-lg transition-colors ${isActive
